    ...
    export default{
        ...
        methods: {
            getData(){
                ...
                axios.get(`http://${port}:3000/api/article?offset=${(this.page-1)*this.pagination.pageSize}&pageSize=${this.pagination.pageSize}`+
            ...
            delArticle(type){
                ...
                if(type=='ok'){
                    axios.delete(`http://${port}:3000/api/article/${this.currentId}`).then((res)=>{
                        if(res.data.err){
                            alert('删除失败')
                        }else{
                            alert('删除成功')
            ...
            removeSelect(){
                let arr=[]
                arr=this.selectArticles.map(i=>axios.delete(`http://${port}:3000/api/article/${i.id}`))         
